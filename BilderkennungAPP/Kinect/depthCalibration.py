...
def DepthCalibration():
    ...
    for k in range(0,const.numberofCalibrations):
        irName = const.irFolder + str(k)+".dat"
        rgbName = const.rgbFolder + str(k)+".dat"
        ...
        rgbImagePoints = rgbImagePoints.reshape((1,np.prod(const.pattern_size),2))
        obj_points = obj_points.reshape((1,np.prod(const.pattern_size),3))
    ...
    #save it
    distanceErrorFile = shelve.open(const.distanceErrorFunction+"/distanceError", 'n')
